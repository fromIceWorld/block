...
class Injector {
    static create(providers, parent = currentInjector) {
...
class StaticInjector {
    constructor(provides = [], desc = '', parent = currentInjector) {
        this.parent = parent;
...
class elementNode extends TNode {
    constructor(tagName, index, dynamicStyle = [], dynamicClasses = [], attributes = {}, events = {}, dynamicAttributes = {}, references = {}, structures = {}, model = {}) {
        ...
        Object.keys(structures).forEach((key) => (this.attributes[AttributeType.structure][key] = new Function(...structures[key])));
        this.finAttributes = Object.assign({}, attributes);
        Object.keys(dynamicAttributes).map((key) => {
            this.attributes[AttributeType.dynamicAttrubute][key] = new Function(...dynamicAttributes[key]);
        });
    }
}
...
class TemplateDynamic extends Array {
    constructor() {
        ...
        this[_f] = {};
        this[_g] = ViewMode.sleep;
        Object.setPrototypeOf(this, TemplateDynamic.prototype);
    }
    injectProviders() {
        var _h;
        let providers = ((_h = Object.getOwnPropertyDescriptor(this[TViewIndex.Class], 'providers')) === null || _h === void 0 ? void 0 : _h.value) || [];
    ...
    updateInput(ctx) {
        ...
        let tNode = this[TViewIndex.TNode] || new elementNode('', Infinity), { finAttributes } = tNode, inputKeys = ctx[InputKeys] || {}, inputChanges = ctx[InputChanges], conflict = new Map();
    ...
    createOutput(ctx) {
        ...
        let host = this[TViewIndex.Host], outputKeys = ctx[EventKeys] || {}, outputEventObj = (ctx[EventChanges] = Object.create({}));
     ...
     */
    initContext() {
        let dir = this[TViewIndex.Class], tokens = dir[InjectToken] || [], providers = tokens.map((token) => { var _h; return (_h = this[TViewIndex.Injector]) === null || _h === void 0 ? void 0 : _h.get(token); }), ctx = new dir(...providers);
...
TViewIndex.Host, _a = TViewIndex.RootElements, TViewIndex.TNode, TViewIndex.LView, TViewIndex.Parent, _b = TViewIndex.Children, _c = TViewIndex.Directives, TViewIndex.Class, _d = TViewIndex.Context, TViewIndex.ComponentDef, TViewIndex.Slots, TViewIndex.Injector, TViewIndex.Module, _e = TViewIndex.InRange, _f = TViewIndex.References, TViewIndex.EmbeddedView, _g = TViewIndex.Mode;
...
class ViewContainer extends TemplateDynamic {
    constructor(index, def, dir) {
        ...
        let currentTView = (this.currentTView = TViewFns.currentTView()), currentLView = currentTView[TViewIndex.LView];
        this[TViewIndex.Host] = currentLView[index + offset$1];
        this[TViewIndex.Class] = dir;
        this[TViewIndex.LView] = new LogicView();
        this[TViewIndex.TNode] = currentTView[index + offset$1];
    ...
    update() {
        ...
        this.updateInput(this[TViewIndex.EmbeddedView]);
        let views = this[TViewIndex.EmbeddedView].OnInputChanges(this[TViewIndex.EmbeddedView][InputChanges]);
        this.diff(views.map((context) => Object.setPrototypeOf(context, this[TViewIndex.Context])));
        TViewFns.popContext();
    }
    diff(viewsContext) {
        var _a;
        for (let i = 0; i < Math.max(viewsContext.length, this.previousContext.length); i++) {
            if (!this.previousContext[i] && viewsContext[i]) {
                let embedded = new embeddedView(this.def, viewsContext[i], this[TViewIndex.Host]);
                embedded[TViewIndex.InRange] = this[TViewIndex.InRange];
                this.childrenView[i] = embedded;
                embedded.install();
                this.previousContext[i] = viewsContext[i];
                this[TViewIndex.Host].after(...Array.from(embedded[TViewIndex.Host].childNodes));
                ...
                this.childrenView[i][TViewIndex.RootElements].map((index) => this.childrenView[i][TViewIndex.LView][index + offset$1].remove());
                this.childrenView[i] = {};
            }
            else if (this.previousContext[i] && viewsContext[i]) {
                // 更新embedded 的上下文
...
class embeddedView extends TemplateDynamic {
    ...
    destroyed() {
        TViewFns.pushContext(this);
        this[TViewIndex.Directives]; const children = this[TViewIndex.Children];
        for (let child of children) {
            let tNode = this[child + offset$1];
...
class TemplateView extends TemplateDynamic {
        ...
        this.$getDefinition = (() => {
            return () => {
                if (!this[TViewIndex.ComponentDef]) {
                    const compilerInstance = this[TViewIndex.Injector].get(compiler);
                    this[TViewIndex.ComponentDef] = compilerInstance.transform(this[TViewIndex.Class]);
                }
    ...
    install() {
        ...
        this.mergeContextAndDecorators(this[TViewIndex.Context]);
        TViewFns.pushContext(this);
        const def = this.$getDefinition(), children = this[TViewIndex.Children];
        // 加载css
        const { styles } = this[TViewIndex.Class], styleDOM = document.createElement('style');
    ...
    update() {
        ...
        // if (conflict.size) {
        TViewFns.pushContext(this);
        let def = this.$getDefinition(), children = this[TViewIndex.Children];
    ...
    destroyed() {
        ...
        Hook(this[TViewIndex.Context], 'OnDestroy');
        (_a = this[TViewIndex.Host]) === null || _a === void 0 ? void 0 : _a.replaceChildren();
        // 卸载style
...
function popContext() {
    let tView = currentTView(), preTView = tView[TViewIndex.Parent];
...
function elementStart(tagName, index) {
    ...
    // 添加静态属性
    addStaticAttributes(tNode.native, tNode.attributes[AttributeType.staticAttribute]);
    resolveDirective(tagName, index);
    processedNodeContext(index);
}
function createNative(tNode, index) {
    let TView = currentTView(), LView = TView[TViewIndex.LView], { tagName, finAttributes } = tNode, slotName = finAttributes['name'], parentTView = TView[TViewIndex.Parent], dom = θd.createElement(tagName);
    while (parentTView instanceof ViewContainer) {
        parentTView = parentTView[TViewIndex.Parent];
    }
    if (tagName == 'slot') {
        let slotsIndex = TView[TViewIndex.Slots], filters = [], slotsDOM = slotsIndex.map((index) => parentTView[TViewIndex.LView][index + offset]);
...
function elementEnd(tagName) {
    const TView = currentTView(), LView = TView[TViewIndex.LView], index = elementStack.pop(), native = LView[index + offset], tNode = TView[index + offset];
...
function updateProp(index, attributes, finAttributes, model) {
    let TView = currentTView(), LView = TView[TViewIndex.LView], native = LView[index + offset], context = TView[TViewIndex.Context];
    Object.keys(attributes).forEach((key) => {
...
function updateInputValue(index, attributes, finAttributes) {
    let TView = currentTView(), context = TView[TViewIndex.Context];
    Object.keys(attributes).forEach((key) => {
...
function updateClass(index, classes, finAttributes) {
    let TView = currentTView(), LView = currentLView(), native = LView[index + offset], context = TView[TViewIndex.Context], result;
...
function updateModelProp(index, attributes, model, finAttributes) {
    const TView = currentTView(), LView = TView[TViewIndex.LView], context = TView[TViewIndex.Context];
...
function updateStyle(index, fns, finAttributes) {
    let TView = currentTView(), LView = currentLView(), native = LView[index + offset], styleMap = native.attributeStyleMap, context = TView[TViewIndex.Context], styleObj = Object.create({});
    fns.forEach((fn) => {
...
function creatText(index) {
    let TView = currentTView(), LView = TView[TViewIndex.LView], { attributes } = TView.$getDefinition(), ctx = TView[TViewIndex.Context], fn = new Function(...attributes[index][AttributeType.text]['content']), text = θd.createTextNode(fn(ctx));
...
function updateText(index) {
    let TView = currentTView(), { content } = TView[index + offset], ctx = TView[TViewIndex.Context], LView = currentLView(), element = LView[index + offset];
...
function createComment(index, content) {
    let TView = currentTView(), LView = TView[TViewIndex.LView], comment = θd.createComment(content);
...
function extractStructures(index, def) {
    ...
    let { attributes } = TNode, has = false, structures = attributes[AttributeType.structure];
...
function resolveDirective(tagName, index) {
    let TView = currentTView(), native = TView[TViewIndex.LView][index + offset], TNode = TView[offset + index];
    let { attributes, directives } = TNode;
    const InRanges = TView[TViewIndex.InRange]() || [], ctx = TView[TViewIndex.Context], [, , staticAttributes, , dynamicAttributes, references, structure] = attributes, localTags = Object.keys(references), viewTagChanges = ctx[ViewChanges]; Object.values(ctx[ViewKeys] || {}); const matchTag = [];
...
function processedNodeContext(index) {
    const TView = currentTView(), LView = TView[TViewIndex.LView], { nodeType } = LView[index + offset];
    let parentIndex;
    let rootElements = TView[TViewIndex.RootElements];
    if (elementStack.length == 0) {
...
function linkParentChild(parentIndex, index) {
    const TView = currentTView(), LView = TView[TViewIndex.LView];
...
class Instruction {
    ...
    resolveElement(element) {
        ...
        if (Object.keys(structures).length) {
            ...
            // 过滤掉第一个结构性指令
            delete copyEle.resolvedAttributes[AttributeType.structure][Object.keys(copyEle.resolvedAttributes[AttributeType.structure])[0]];
     ...
     */
    resolveText(element) {
        this.attributes[this.index] = Array.from(new Array(AttributeType.length), () => Object.create(null));
        this.instructionParams.add('creatText');
        let text = this.attributes[this.index][AttributeType.text], hasInterpolation = false, [start, end] = this.configuration.interpolationSyntax, interpolationRegExp = new RegExp(`${start}\\s*[a-zA-Z0-9!.'"\\[\\]]*\\s*${end}`, 'g'), { content } = element;
...
export { Application, AttributeType, CheckDetectChange, Component, Decorator, Directive, EventChanges, EventEmitter, EventKeys, Hook, Inject, InjectToken, InjectionToken, Injector, Input, InputChanges, InputKeys, Instruction, LogicView, Module, Output, PlatformBrowserDynamic, PlatformRef, StaticInjector, TViewFns, TViewIndex, TemplateDirective, TemplateDynamic, TemplateEmbedded, TemplateView, ViewChanges, ViewChild, ViewContainer, ViewKeys, ViewMode, bootstrapView, collectRunDependency, commentNode, compiler, componentFromModule, createPlatform, currentInjector, elementNode, elementType, embeddedView, formControl, formGroup, formsModule, instructionIFrameStates, offset$1 as offset, registerApplication, resolveSelector, routeConfig, textNode };
