  ...
  resolveElement(element: ElementTNode) {
    ...
    if (Object.keys(structures).length) {
      ...
      delete copyEle.resolvedAttributes[AttributeType.structure][
        Object.keys(copyEle.resolvedAttributes[AttributeType.structure])[0]
      ];
      ...
      if (attributes[i + 1] == "=") {
        switch (prefix) {
          case addAttributeMark:
            let [styles, classes, , , attrs] = this.extractDynamiceAttributes(
  ...
  extractDynamiceAttributes(dynamicKey: string, value: string) {
    ...
    switch (dynamicKey) {
      ...
      case "class":
        ...
        break;
      default:
        result[AttributeType.dynamicAttrubute][dynamicKey] = contextValue;
        break;
    }
    return result;
  }
